import {
  ...
  OnDestroy,
  ViewChild,
  ContentChild, ChangeDetectionStrategy
} from '@angular/core';
...
export class NgxGauge implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private _updateSize() {
    ...
    this._renderer.setStyle(this._label.nativeElement,
      'transform', 'translateY(' + (this.fontSize ? ((this.size - (this.size / 13)) / 2) + this.fontSize : (this.size / 3 * 2 - this.size / 13 / 4)) + 'px)');
    this._renderer.setStyle(this._reading.nativeElement,
      'transform', 'translateY(' + (this.fontSize ? (this.size - this.fontSize) / 2 : (this.size / 2 - this.size * 0.22 / 2)) + 'px)');
  }
