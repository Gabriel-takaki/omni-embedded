...
export class FunnelGraph {
    constructor(options) {
        console.log('inicializando grafico', options);
        ...
        this.labels = this.getLabels(options);
        this.subLabels = FunnelGraph.getSubLabels(options);
        this.values = this.getValues(options);
    ...
    getLabels(options) {
        const result = [];
        for (const pipe of options.data) {
            result.push(pipe.name);
        }
        return result;
    }
    ...
    addLabels() {
        ...
        this.percentages.forEach((percentage, index) => {
            ...
            labelElement.setAttribute('class', `svg-funnel-js__label flex-column label-${index + 1}`);

            const title = document.createElement('div');
            title.setAttribute('class', 's12 font-light');
            title.textContent = this.labels[index] || '';

            const value = document.createElement('div');
            value.setAttribute('class', 'flex-row align-items-center overflow-hidden mrg4T');
            value.innerHTML = `
                <div class="font-semi-bold fg-black-lighten-1 s18" matTooltip="Total de oportunidades que passaram por essa etapa.">
                    ${this.values[index]}
                </div>
                <div class="mrg12L label-big label-info">
                    ${percentage.toString()}%
                </div>
                <div class="mrg6L flex-column">
                    ...
                    <div class="mrg2T label label-success">
                        10%
                    </div>
                </div>
            `;
    ...
    updateData(d) {
        ...
        this.values = [];
        this.percentages = [];
        this.labels = this.getLabels({data: d});
        ...
        if (typeof d.values !== 'undefined') {
            if (Object.prototype.toString.call(d.values[0]) !== Object.prototype.toString.call(this.values[0])) {
                ...
                this.values = this.getValues({data: d});
                this.makeSVG();
            } else {
                this.values = this.getValues({data: d});
            }
